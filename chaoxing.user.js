...
(function (vue, pinia, rxjs, md5, ElementPlus) {
  'use strict';
  // === Hook playbackRate 阻止超星 JS 重置倍速 ===
  try {
    var _prDesc = Object.getOwnPropertyDescriptor(HTMLMediaElement.prototype, 'playbackRate');
    if (_prDesc && _prDesc.configurable) {
      var _prOrigSet = _prDesc.set;
      var _prOrigGet = _prDesc.get;
      Object.defineProperty(HTMLMediaElement.prototype, 'playbackRate', {
        get: function() { return _prOrigGet.call(this); },
        set: function(rate) {
          if (window.__cxSpeedOverride && this.__cxUserSpeed && this.__cxUserSpeed > 1 && rate !== this.__cxUserSpeed) {
            _prOrigSet.call(this, this.__cxUserSpeed);
            return;
          }
          _prOrigSet.call(this, rate);
        },
        configurable: true
      });
    }
  } catch(e) { console.warn('[CX] playbackRate hook failed:', e); }

  // === Hook addEventListener 阻止 ratechange 事件监听 ===
  try {
    var _origAddEventListener = HTMLMediaElement.prototype.addEventListener;
    HTMLMediaElement.prototype.addEventListener = function(type, listener, options) {
      if (type === 'ratechange' && window.__cxSpeedOverride) {
        return;
      }
      return _origAddEventListener.call(this, type, listener, options);
    };
  } catch(e) { console.warn('[CX] addEventListener hook failed:', e); }

  // === 动态注入 videojs 钩子到 iframe ===
  var _vjsHookTimer = setInterval(function() {
    try {
      var _frames = document.querySelectorAll('iframe');
      for (var _fi = 0; _fi < _frames.length; _fi++) {
        var _fw = _frames[_fi].contentWindow;
        if (_fw && !_fw.__cxVjsHooked && _fw.videojs) {
          _fw.__cxVjsHooked = true;
          var _origVjs = _fw.videojs;
          _fw.videojs = function() {
            var player = _origVjs.apply(this, arguments);
            if (player && player.on) {
              var _playerOrigOn = player.on;
              player.on = function(evt, fn) {
                if (evt === 'ratechange') return player;
                return _playerOrigOn.call(this, evt, fn);
              };
            }
            if (player && player.playbackRate) {
              var _origPR = player.playbackRate;
              player.playbackRate = function(rate) {
                if (arguments.length > 0 && window.__cxSpeedOverride && window.__cxSpeedRate > 1) {
                  return _origPR.call(this, window.__cxSpeedRate);
                }
                return _origPR.apply(this, arguments);
              };
            }
            return player;
          };
          for (var _vk in _origVjs) {
            if (_origVjs.hasOwnProperty(_vk)) _fw.videojs[_vk] = _origVjs[_vk];
          }
        }
      }
    } catch(e) {}
  ...
  // === 防暂停：拦截 mouseout 事件注册 ===
  try {
    var _origAddEvt = EventTarget.prototype.addEventListener;
    EventTarget.prototype.addEventListener = function(type, listener, options) {
      if (type === 'mouseout' && window.__cxSpeedOverride) {
        var _src = '';
        try { _src = (listener && listener.toString ? listener.toString() : '').slice(0, 200); } catch(e) {}
        if (_src.indexOf('video') !== -1 || _src.indexOf('pause') !== -1 || _src.indexOf('player') !== -1) {
          return;
        }
      }
      return _origAddEvt.call(this, type, listener, options);
    };
  } catch(e) { console.warn('[CX] mouseout hook failed:', e); }

  // === 防暂停：阻止 visibilitychange 传播 ===
  try {
    document.addEventListener('visibilitychange', function(e) {
      if (window.__cxSpeedOverride) { e.stopImmediatePropagation(); e.stopPropagation(); }
    }, true);
  } catch(e) {}

  // === 防暂停：重写 video.pause() 自动恢复播放 ===
  try {
    var _origPause = HTMLMediaElement.prototype.pause;
    HTMLMediaElement.prototype.pause = function() {
      var ret = _origPause.call(this);
      if (window.__cxSpeedOverride && this.__cxUserSpeed && this.__cxUserSpeed > 1 && !this.ended && this.currentTime > 0) {
        var _v = this;
        setTimeout(function() {
          if (_v.paused && !_v.ended && _v.__cxUserSpeed && _v.__cxUserSpeed > 1) {
            _v.play().catch(function(){});
          }
        }, 300);
      }
      return ret;
    };
  } catch(e) { console.warn('[CX] pause hook failed:', e); }

  // === 防暂停：屏蔽 visibilitychange + blur 事件 ===
  try {
    var _origDocAddEvt = Document.prototype.addEventListener;
    Document.prototype.addEventListener = function(type, listener, options) {
      if ((type === 'visibilitychange') && window.__cxSpeedOverride) return;
      return _origDocAddEvt.call(this, type, listener, options);
    };
    window.addEventListener('blur', function(e) {
      if (window.__cxSpeedOverride) { e.stopImmediatePropagation(); e.stopPropagation(); }
    }, true);
    window.addEventListener('mouseout', function(e) {
      if (window.__cxSpeedOverride && (!e.relatedTarget || e.relatedTarget === null)) {
        e.stopImmediatePropagation();
        e.stopPropagation();
      }
    }, true);
  } catch(e) { console.warn('[CX] visibility/blur hook failed:', e); }
    ...
    state: () => {
        ...
        otherParams: {
          ...
          params: [
            ...
            },
            {
              ...
              value: false,
              type: "boolean",
              tooltip: "不确保开启倍数后视频能够正常完成，通常有30~50%左右的几率被判断异常观看时长而打回，更有甚者，如若多次，可能会出现封号等问题。如果你要使用此功能，请务必明确风险，所有由你的决定造成的不当风险不应归咎于开发者，开发者与你的风险操作而导致的利益损失问题无关"
            },
            {
              name: "倍速值",
              value: 2,
      ...
      if (storedConfig) {
        try {
          ...
          // 合并默认参数：补齐新版新增的配置项
          const defParams = defaultConfig.otherParams.params;
          const curParams = parsedStoredConfig.otherParams.params || [];
          for (let i = 0; i < defParams.length; i++) {
            if (!curParams[i]) { curParams[i] = defParams[i]; }
          }
          parsedStoredConfig.otherParams.params = curParams;
              ...
              (vue.openBlock(true), vue.createElementBlock(vue.Fragment, null, vue.renderList(item.params, (param, index2) => {
                return vue.openBlock(), vue.createElementBlock(vue.Fragment, null, [
                  param.type === "boolean" ? (vue.openBlock(), vue.createBlock(_component_el_checkbox, {
                    ...
                    default: vue.withCtx(() => [
                      vue.createTextVNode(vue.toDisplayString(param.name), 1),
                      param.tooltip ? (vue.openBlock(), vue.createBlock(vue.resolveComponent("el-tooltip"), {
                        content: param.tooltip, placement: "top", effect: "dark", "show-after": 300,
                        "popper-style": { "z-index": "100004" }
                      }, {
                        default: vue.withCtx(() => [vue.createTextVNode(" ⓘ")]),
                        _: 1
                      }, 8, ["content"])) : vue.createTextVNode("")
                    ]),
                    _: 1
                  }, 8, ["modelValue", "onUpdate:modelValue"])) : (vue.openBlock(), vue.createBlock(_component_el_form_item, {
                    key: 1,
                    label: param.name,
                    required: ""
                  }, {
              ...
              return vue.openBlock(), vue.createElementBlock(vue.Fragment, { key: index }, [
                item.type === "boolean" ? (vue.openBlock(), vue.createBlock(_component_el_checkbox, {
                  ...
                  default: vue.withCtx(() => [
                    ...
                    item.tooltip ? (vue.openBlock(), vue.createBlock(vue.resolveComponent("el-tooltip"), {
                      content: item.tooltip, placement: "top", effect: "dark", "show-after": 300,
                      "popper-style": { "z-index": "100004" }
                    }, {
                      default: vue.withCtx(() => [vue.createTextVNode(" ⓘ")]),
                      _: 1
                  ...
                  default: vue.withCtx(() => [
                    vue.createVNode(_component_el_input_number, {
                      modelValue: item.value,
                      "onUpdate:modelValue": ($event) => item.value = $event,
                      min: item.name.includes('倍速值') ? 1 : 3,
                      max: item.name.includes('倍速值') ? 16 : 100,
                      step: 1,
                      size: "small"
                    }, null, 8, ["modelValue", "onUpdate:modelValue"])
                  ]),
  ...
  const _hoisted_acc = /* @__PURE__ */ vue.createElementVNode("p", { style: { "margin-top":"12px","padding":"10px","background":"#f0f9ff","border-radius":"6px","font-size":"12px","border":"1px solid #bae6fd","line-height":"1.6" } }, "题库准确率：基于116道多学科真题实测（2026.06），综合准确率90%+。简答题85~90%，多选题95%+，填空题100%。DeepSeek V4 + 联网搜索 + 提示词优化。", -1);
  ...
  function _sfc_render$cache(_ctx,_cache) {
    const btnStyle = { "font-size":"11px","padding":"4px 10px" };
    return vue.openBlock(), vue.createElementBlock("div", { style: { "font-size":"12px" } }, [
      vue.createElementVNode("div", { style: { "display":"flex","align-items":"center","gap":"8px","margin-bottom":"8px" } }, [
        vue.createElementVNode("span", { style:{"white-space":"nowrap"} }, "共 "+_ctx.count+" 条", -1),
        vue.withDirectives(vue.createElementVNode("input", { type:"text", placeholder:"搜索...", style:{"flex":"1","min-width":"0","padding":"5px 8px","border":"1px solid #ddd","border-radius":"4px","font-size":"12px"}, onInput: _cache[0] || (_cache[0] = ($event) => _ctx.searchText = $event.target.value) }, null, 512), [[vue.vModelText, _ctx.searchText]])
      ]),
      vue.createElementVNode("div", { style: { "display":"flex","flex-wrap":"wrap","gap":"4px","margin-bottom":"10px" } }, [
        ...
        vue.createVNode(vue.resolveComponent("el-button"), { size:"small", type:"info", onClick:_ctx.exportCfg, style:btnStyle }, { default: vue.withCtx(() => [vue.createTextVNode("导出配置")]) }),
        vue.createVNode(vue.resolveComponent("el-button"), { size:"small", type:"primary", onClick:_ctx.importCfg, style:btnStyle }, { default: vue.withCtx(() => [vue.createTextVNode("导入配置")]) }),
      ...
      vue.createElementVNode("div", { style:{"max-height":"45vh","overflow":"auto"} }, [
        (vue.openBlock(), vue.createElementBlock(vue.Fragment, null, vue.renderList(_ctx.filtered, (entry) => {
          return vue.createElementVNode("div", { key: entry.key, style:{"padding":"3px 0","border-bottom":"1px solid #eee","font-size":"11px","display":"flex","justify-content":"space-between","align-items":"flex-start"} }, [
            vue.createElementVNode("span", { style:{"flex":"1","word-break":"break-all","overflow-wrap":"break-word","min-width":"0"} }, (Array.isArray(entry.value) ? entry.value.join(', ') : String(entry.value)), -1),
  ...
  function decrypt(iframeDocument) {
    ...
    for (let i = 19968; i < 40870; i++) {
      ...
      if (table[hash] != null) match[i] = table[hash];
    }
  ...
  const getAnswerFrom = async (question, url2) => {
    ...
    const strictMode = configStore.platformParams.cx.parts[0].params[4]?.value || false;
  ...
  const getCachedAnswer = (question) => {
    try {
      const key = 'cx_ans_' + md5(question.title + question.type + question.workType);
    ...
    constructor(type, iframe) {
      ...
      __publicField(this, "init", async () => {
        ...
        if (this.questions.length) {
          ...
          for (const [index, question] of this.questions.entries()) {
            // 跳过已答题目
            const _cfg = useConfigStore();
            const cached = getCachedAnswer(question);
            if (cached && _cfg.platformParams.cx.parts[0].params[3]?.value) {
              question.answer = cached;
              this.fillQuestion(question);
              this.addLog(`已跳过已回答题目 · 第${index + 1}道`, 'info');
              this.correctNum += 1;
              this.addQuestion(question);
              continue;
            }
            const answerData = await getAnswer(question);
            const conf = answerData.data?.confidence || 'high';
            if (answerData.code === 200 && conf === 'high') {
              ...
              if (ok) {
                question.answer = answerData.data.answer;
                this.fillQuestion(question);
                this.addLog(`第${index + 1}道题已确认(存疑)`, "warning");
                this.correctNum += 1;
                setCachedAnswer(question, answerData.data.answer);
              } else {
                question.answer = [];
                this.addLog(`第${index + 1}道题用户拒绝AI建议`, "info");
              }
      ...
      __publicField(this, "fillQuestion", (question) => {
        ...
        if (question.type === "0" || question.type === "1") {
          const isMulti = question.type === "1";
          const answers = question.answer;
          (async () => {
            for (let ai = 0; ai < answers.length; ai++) {
              const answer = this.removeHtml(answers[ai]).trim();
              let matched = false;
              // 遍历选项，模糊匹配
              for (const key in question.options) {
                const optText = key.trim();
                // 多种匹配策略
                if (optText === answer ||
                    optText.includes(answer) ||
                    answer.includes(optText) ||
                    optText.replace(/^[A-Z][.、．)\s]+/, '') === answer ||
                    answer.replace(/^[A-Z][.、．)\s]+/, '') === optText) {
                  matched = true;
                  if (["zj", "zy"].includes(this.type)) {
                    const optionElement = question.options[key];
                    if (optionElement.getAttribute("aria-checked") === "true") continue;
                    optionElement == null ? void 0 : optionElement.click();
                  } else if (["ks"].includes(this.type)) {
                    const optionElement = question.options[key];
                    if (optionElement.querySelector(".check_answer")) continue;
                    if (optionElement.querySelector(".check_answer_dx")) continue;
                    optionElement == null ? void 0 : optionElement.click();
                  }
                  if (isMulti && ai < answers.length - 1) {
  ...
  const useCxChapterLogic = () => {
    ...
    const init = () => {
      ...
      // 防止鼠标移出视频区域暂停（mouseout 拦截）
      document.addEventListener('mouseout', (e) => { if (e.target.tagName === 'VIDEO' || e.target.tagName === 'IFRAME') { e.stopImmediatePropagation(); e.stopPropagation(); } }, true);
      ...
      return new Promise((resolve) => {
        ...
        const intervalId = setInterval(async () => {
          ...
          if (mediaElement && !isExecuted) {
            ...
            const speedEnabled = configStore.otherParams.params[3]?.value;
            const speed = configStore.otherParams.params[4]?.value;
            // === 直接注入脚本到 iframe 内部，确保在 iframe 上下文中执行 ===
            if (speedEnabled && speed && speed > 1) {
              window.__cxSpeedOverride = true;
              window.__cxSpeedRate = speed;
              mediaElement.__cxUserSpeed = speed;
              mediaElement.playbackRate = speed;
              mediaElement.defaultPlaybackRate = speed;
              logStore.addLog(`已启用 ${speed}x 倍速`, "warning");
              try {
                var _injDoc = iframeDocument;
                if (_injDoc && !_injDoc.__cxInjected) {
                  _injDoc.__cxInjected = true;
                  var _scr = _injDoc.createElement('script');
                  _scr.textContent = '(function(){' +
                    'var S=' + speed + ';' +
                    'var _d=Object.getOwnPropertyDescriptor(HTMLMediaElement.prototype,"playbackRate");' +
                    'if(!_d||!_d.configurable)return;' +
                    'var _nSet=_d.set,_nGet=_d.get,_realRates=new WeakMap();' +
                    // 1. 欺骗 playbackRate: getter永远返回1.0 → 超星以为正常速度
                    'Object.defineProperty(HTMLMediaElement.prototype,"playbackRate",{' +
                    'get:function(){return 1.0;},' +
                    'set:function(r){_realRates.set(this,S);_nSet.call(this,S);},' +
                    'configurable:true});' +
                    // 2. 拦截 ratechange 事件 → 超星收不到速度变化通知
                    'var _evDispatch=EventTarget.prototype.dispatchEvent;' +
                    'EventTarget.prototype.dispatchEvent=function(e){' +
                    'if(e&&e.type==="ratechange"&&this instanceof HTMLMediaElement)return true;' +
                    'return _evDispatch.call(this,e);};' +
                    // 3. 阻止 pause() → 超星无法暂停（当倍速激活时）
                    'var _pause=HTMLMediaElement.prototype.pause;' +
                    'HTMLMediaElement.prototype.pause=function(){' +
                    'if(_realRates.get(this)&&_realRates.get(this)>1&&!this.ended&&this.currentTime>0)return;' +
                    'return _pause.call(this);};' +
                    // 4. 1s轮询保活 → 确保倍速和播放状态
                    'setInterval(function(){' +
                    'var vs=document.querySelectorAll("video");' +
                    'for(var i=0;i<vs.length;i++){var v=vs[i];if(v.currentTime>0&&!v.ended){' +
                    '_nSet.call(v,S);if(v.paused)v.play().catch(function(){});}' +
                    '}},1000);' +
                    // 5. 阻止 mouseout 注册
                    'var _ael=EventTarget.prototype.addEventListener;' +
                    'EventTarget.prototype.addEventListener=function(t,l,o){' +
                    'if(t==="mouseout"){try{var s=l.toString().slice(0,200);' +
                    'if(s.indexOf("pause")!==-1||s.indexOf("video")!==-1)return;}catch(e){}}' +
                    'return _ael.call(this,t,l,o);};' +
                    // 6. 阻止 visibilitychange
                    'document.addEventListener("visibilitychange",function(e){e.stopImmediatePropagation()},true);' +
                    '})();';
                  (_injDoc.head || _injDoc.documentElement).appendChild(_scr);
                }
              } catch(e) { console.warn('[CX] iframe inject err:', e); }
            }
    ...
    constructor() {
      ...
      __publicField(this, "init", async () => {
        ...
        if (this.questions.length) {
          this.addLog(`成功解析到${this.questions.length}个题目`, "primary");
          for (const [index, question] of this.questions.entries()) {
            const _cfg2 = useConfigStore();
            const cached = getCachedAnswer(question);
            if (cached && _cfg2.platformParams.cx.parts[0].params[3]?.value) {
              question.answer = cached;
              this.fillQuestion(question);
              this.addLog(`已跳过已回答题目 · 第${index + 1}道`, 'info');
              this.addQuestion(question);
            ...
            const answerData = await getAnswer(question);
            const conf2 = answerData.data?.confidence || 'high';
            if (answerData.code === 200 && conf2 === 'high') {
              question.answer = answerData.data.answer;
              this.fillQuestion(question);
              this.addLog(`第${index + 1}道题搜索成功`, "success");
              this.addLog(`剩余次数:${answerData.data.num}`, "primary");
      ...
      __publicField(this, "fillQuestion", (question) => {
        ...
        if (question.type === "单选题" || question.type === "多选题") {
          question.answer.forEach((answer) => {
            const a = this.removeHtml(answer).trim();
            let done = false;
            for (const key in question.options) {
              const k = key.trim();
              if (k === a || k.includes(a) || a.includes(k) || k.replace(/^[A-Z][.、．)\s]+/,'')===a || a.replace(/^[A-Z][.、．)\s]+/,'')===k) {
                question.options[key].click();
                done = true;
                break;
              }
            ...
            if (!done) {
              for (const key in question.options) {
                if (key.trim().includes(a.substring(0,2))) { question.options[key].click(); break; }
              }
            }
          });
